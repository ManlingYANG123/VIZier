...
const B_FULL = [
  baseCritique({
    ...
    object: "chart",
    problem: "difficult to read precisely",
    title: "Expose each monthly velocity observation",
    issue: "The task-velocity lines communicate direction but hide the individual monthly observations.",
    suggestion: "Add compact points to both task-velocity series.",
    rationale: "Points make month-by-month inspection easier while preserving the completed-versus-target comparison.",
    evidence: "The task-velocity mark currently disables points even though the x-axis contains seven discrete months.",
    priority: "high",
  ...
  baseCritique({
    id: "practice-b-department-labels",
    dimension: "text",
    object: "axis label",
    problem: "crowded",
    title: "Give department labels more room",
    issue: "The department comparison leaves limited room for longer team names.",
    suggestion: "Angle the department labels slightly and increase their size.",
    rationale: "A modest angle preserves the chart footprint while making department names easier to scan.",
    evidence: "The department-tasks chart places five department labels along its x-axis.",
  ...
  baseCritique({
    id: "practice-b-burndown-color",
    dimension: "color",
    object: "color",
    problem: "overly saturated",
    title: "Quiet the burndown accent",
    issue: "The sprint burndown uses a strong orange that pulls attention away from the broader delivery overview.",
    suggestion: "Use a quieter, deeper orange for the burndown line.",
    rationale: "A restrained accent keeps the time-series visible without overpowering project status and team velocity.",
    evidence: "The sprint-burndown line uses the dashboard's most saturated warm accent.",
    tileId: "sprint-burndown",
    proposal: {
      kind: "edit-spec",
      edits: [{ op: "set", path: ["mark", "color"], value: "#9a4c18" }],
    },
  }),
];
...
const PRESETS = {
  ...
  B: {
    materialCode: "B",
    context: "Help delivery leaders track task momentum, sprint progress, and project health across departments so they can identify where follow-up is needed.",
    editedContext: "Help delivery leaders compare task momentum, sprint progress, and project health across departments, emphasizing where work is at risk.",
    audience: "Delivery leaders monitoring cross-department project health.",
    document: {
      filename: "B_tableau-dashboard-best-practices.pdf",
      constraints: [
        { id: "practice-b-rule-1", category: "Layout", rule: "Keep related delivery and project-health views aligned to a consistent dashboard grid." },
    ...
    focused: {
      request: "Should I change the layout?",
      answer: "Yes. Giving the two time-based views more width would make delivery momentum the primary reading path while department and status summaries remain available as supporting context.",
      critiques: [baseCritique({
        id: "practice-b-layout",
        dimension: "layout",
        object: "layout",
        problem: "unbalanced",
        title: "Lead with delivery momentum",
        issue: "All four views currently receive equal width even though the two time-based views carry the primary delivery narrative.",
        suggestion: "Give task velocity and sprint burndown a wider analytical column, with department and status as a narrower supporting column.",
        rationale: "A stronger width hierarchy makes delivery momentum the clear reading path without removing the supporting breakdowns.",
        evidence: "Task velocity, department tasks, sprint burndown, and project status currently occupy an equal two-by-two grid.",
        priority: "high",
        proposal: {
          kind: "edit-layout",
          composition: "delivery-hero-grid",
          layoutByTile: {
            "task-velocity": { x: 28, y: 144, w: 650, h: 258 },
            "department-tasks": { x: 702, y: 144, w: 370, h: 258 },
            "sprint-burndown": { x: 28, y: 448, w: 650, h: 272 },
            "project-status": { x: 702, y: 448, w: 370, h: 272 },
          },
        },
        answer: "A wider temporal column would create a clearer delivery-focused scan order.",
        requestRelevance: "direct",
      })],
    },
    local: {
      request: "Is this title clear enough?",
      answer: "It identifies the workspace, but not the delivery decisions the dashboard supports. Naming momentum and project health would orient readers more quickly.",
      title: "Delivery Momentum and Project Health",
    },
    singleCritiqueId: "practice-b-trend-points",
    batchCritiqueIds: ["practice-b-department-labels", "practice-b-burndown-color"],
  },
};
