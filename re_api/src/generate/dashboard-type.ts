...
const GUIDANCE: Record<DashboardType, string> = {
  ...
  infographic:
    "DASHBOARD GENRE = infographic (narrative). It tells one story and should deliver an explicit conclusion. Prioritize a clear headline takeaway, guided reading order, interpretive context and annotation, and polished visual design. Do NOT require interactive affordances (cross-filters, tooltips, filter controls) or fine-grained self-service detail — this artifact communicates, it does not support open exploration. Centered short callouts and hero statistics are legitimate narrative composition choices; do not label their alignment a readability defect unless the actual copy is long, crowded, clipped, or lacks a stable reading edge. Do not spend critique slots on near-identical editorial rewrites or replacing valid decade-level narrative wording with an exact year unless the coarser wording is genuinely false or decision-relevant precision is lost.",
  executive:
